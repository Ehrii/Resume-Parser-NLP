...
def update_skills_file_from_db():
    ...
    for job_skills in job_skills_results:
        skills_list = [skill.strip() for skill in job_skills[0].split(',')]
...
def extract_skills(text):
    # First, update the skills.txt with job skills from the database
    update_skills_file_from_db()
    ...
    # Return True for the warning if there's a skills section but no skills were found
...
#     # Function to get education level
...
#         # If job education requirement is not in our hierarchy, skip this job
#         if job_required_education_level == -1:
#             continue

#         # Check which education levels meet or exceed the job requirement
